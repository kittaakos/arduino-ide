...
export namespace ErrorSource {
  export function content(source: ErrorSource): string {
...
export function tryParseError(source: ErrorSource): CoreError.ErrorLocation[] {
  const { sketch } = source;
  const content = ErrorSource.content(source);
  if (sketch) {
    return tryParse(content)
      ...
      .reduce((acc, curr) => {
        ...
        if (existingRef) {
          existingRef.rangesInSource.push(...curr.rangesInSource);
        } else {
          acc.push(curr);
        }
        return acc;
...
interface ParseResult {
  ...
  readonly rangeInSource?: Range | undefined;
}
...
function toErrorInfo({
  ...
  return {
    ...
    rangesInSource: rangeInSource ? [rangeInSource] : [],
  };
}
...
export function tryParse(content: string): ParseResult[] {
  ...
  return Array.from(content.matchAll(re) ?? [])
    .map((match) => {
      ...
      const rangeInSource = findRangeInSource(
        start,
